...
/**
 * Minimal Markdown → HTML for digest bodies.
 * Handles: paragraphs, **bold**, bullet lists (- or •).
 */
    ...
    .map((block) => {
      ...
      if (isList) {
        const items = lines
          .map((l) => {
            let c = l.replace(/^\s*[-•●]\s*/, "");
            c = escapeHtml(c).replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
            return `<li>${c}</li>`;
          })
          .join("");
        return `<ul class="list-disc list-inside space-y-1 ml-1">${items}</ul>`;
      }
